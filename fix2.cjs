...
const regexTimeline = /\/\/ 1\. GRIND \([\s\S]*?\/\/ 7\. CUP \-\> SHOP \([\s\S]*?6\.75\);/;
...
if (!regexTimeline.test(code)) {
    console.error("Timeline pattern not found");
    process.exit(1);
}

code = code.replace(regexTimeline, newTimeline);

// Now fix the Bloom JSX
...
if (!code.includes(oldBloomJSX)) {
    console.error("Bloom JSX pattern not found");
    process.exit(1);
}
