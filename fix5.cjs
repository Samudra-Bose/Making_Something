const fs = require('fs');
let code = fs.readFileSync('src/worlds/Brew.tsx', 'utf8');

const startIndex = code.indexOf('// 1. GRIND');
const endIndex = code.indexOf('    });\\r\\n\\r\\n    return () => mm.revert();', startIndex) !== -1 ? code.indexOf('    });\\r\\n\\r\\n    return () => mm.revert();', startIndex) : code.indexOf('    });\\n\\n    return () => mm.revert();', startIndex);

if (startIndex === -1 || endIndex === -1) {
    console.error('Could not find start or end index for timeline', startIndex, endIndex);
    process.exit(1);
}
...
code = code.substring(0, startIndex) + newTimeline + code.substring(endIndex);

// Bloom JSX replace
const startBloom = code.indexOf('{/* 3. BLOOM');
const endBloom = code.indexOf('</div>', startBloom) !== -1 ? code.indexOf('</div>', startBloom) + 6 : -1;
const newBloomJSX = \`{/* 3. BLOOM — unique image: pour-over bloom moment */}
...
code = code.substring(0, startBloom) + newBloomJSX + code.substring(endBloom);
